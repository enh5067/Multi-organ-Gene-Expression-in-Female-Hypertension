...
sim_error = function(datHMF=NULL,Nr=NULL,tlen=NULL,rateConst=NULL,ic=NULL,dt=NULL,epsilon=NULL,numPar=NULL){
  ...
  for(sims in 1:ncol(rateConst)){ #nlambda number of columns

    par = rateConst[,sims]

    # Jacobian matrix, J
    
    J = matrix(par,sqrt(length(par)),sqrt(length(par)),byrow = T)
#browser()
    # implement simulation
    time <- seq(0,tlen,by=dt)
    #time <- seq(0,tlen,length.out = 100)
    out = deSolve::ode(y=ic, times=time, parms=J, func=odemodel, method=lsoda, rtol= 1e-9, atol= 1e-9, hini = dt, hmax = dt)
    #browser()
    out1 = out[,-1]
    out1[!is.finite(out1)] = 0
    if(dim(out1)[1] < length(time)){
      out1 = matrix(0,length(time),ncol = ncol(out1))    
      #errors[sims]  = 1e10
    }
    
    out = cbind(out[,1], out1)
    
    # loop through annotation/feature combinations to compute errors
    sim_time = out[,1] + min(datHMF[,1])
    #browser()
    
    # err = 0
    # for(ii in 1:Nr){
    #   indorg = which(datHMF[,2] == unique(datHMF[,2])[ii])
    #   orgcols = ((ii-1)*(ncol(datHMF)-2)+1):((ncol(datHMF)-2)*ii) + 1
    #   expr_time = as.numeric(datHMF[indorg,1])
    #   ind_sim_time = unlist( sapply(expr_time,function(x){which(abs(x-sim_time) == min(abs(x-sim_time)))}))
    #   sum_sq_err = apply( ( datHMF[indorg,3:ncol(datHMF)] - out[ind_sim_time,orgcols] )^2, 2, sum )
    #   #
    #   err = err+ sum ( sum_sq_err )
    # } #Nr loop
    # 
    # errs = err /(apply(out[,-1],2,var) + epsilon)    
    # errs[!is.finite(errs)] = 1e10
    # errors[sims] = sum(errs)
    ...
    for(ii in 1:Nr){
      ...
      sum_sq_err = apply( ( datHMF[indorg,3:ncol(datHMF)] - out[ind_sim_time,orgcols] )^2, 2, sum )
      #
      errs = sum_sq_err / (apply(out[,orgcols],2,var) + epsilon)
...
#' Format error
#'
#' This function reformats the simulation error data from a list to a data.frame.
...
sim_best = function(best_params=NULL,mRange_best=NULL,tsim=NULL,numPar=NULL,XY=NULL,datHMF=NULL,ic=NULL, dt = NULL, nlambda = 10){

  XY = XY
  numPar = numPar
  Yreg = get_Yreg(datHMF,XY$Y,numPar,mRange_best)
  Xreg = get_Xreg(datHMF,XY$X,numPar,mRange_best)
  fit <- glmnet::glmnet( Xreg, Yreg, family="gaussian",intercept=F,standardize=F,
                 alpha = best_params$alpha_value, lambda = best_params$lambda_value)
  # fit <- glmnet::glmnet( Xreg, Yreg, family="gaussian",intercept=F,standardize=F,
  #                alpha = best_params$alpha_value, nlambda = nlambda)
  
  tsim = tsim
  par = fit$beta
  # par = (get_rateConst(fit,nlambda))$beta
  # par = par[,best_params$lambda_index]
  # sdCut = 2
  # med <- median(par,na.rm=T)
  # dev <- sd(par,na.rm=T)
  # ind <- which( abs(par) > (med+sdCut*dev) )
  # Ypar = par*0
  # Ypar[ind] = par[ind]/max(abs(par))
  # 
  # Jacobian matrix, J
  J = matrix(par,sqrt(length(par)),sqrt(length(par)),byrow = T)
  
  # implement simulation
  sim = deSolve::ode(y=ic, times=tsim, parms=J, func=odemodel, method=lsoda, rtol= 1e-9, atol= 1e-9, hini = dt, hmax = dt)
   #browser()

  # annotate simulation and parameter data
  anns = get_annotations(sim,J, datHMF)
...
#' @return list(sim,J)
#' @examples best_params = select_best(simulate_error)
get_annotations = function(sim=NULL,J=NULL, datHMF=NULL){
